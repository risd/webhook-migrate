...
function help (key) {
  console.log([
    ...
    (key) ? "" : false,
    "migrate <backup> <migrated> --from=<migrateFrom> --url=<uploadUrl> \\",
    "        --siteName=<siteName> --secretKey=<secretKey>",
    "        --requests=<requests>",
    "",
    "backup         path to local json file resulting from running",
    "               `wh preset-build-all` in the project to migrate from.",
    "               ",
    "migrated       path to local json file where the migrated data will",
    ...
    "               ",
    "               ",
    "requests       Optional path to local json file containingan array of requestItem",
    "               objects to be passed into migrate.",
